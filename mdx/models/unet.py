...
from mdx.utils.pytorch_utils import Base, Subband, act, init_bn, init_layer
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.conv_block2 = ConvBlock(out_channels * 2, out_channels, kernel_size, activation, momentum)
    ...
    def forward(self, input_tensor: torch.Tensor, concat_tensor: torch.Tensor) -> torch.Tensor:
        r"""Forward data into the module.
...
class UNet(nn.Module, Base):
    def __init__(self, input_channels: int, target_sources_num: int):
        ...
        self.after_conv2 = nn.Conv2d(
            in_channels=32,
            out_channels=target_sources_num * input_channels * self.K * self.subbands_num,
            kernel_size=(1, 1),
            stride=(1, 1),
            padding=(0, 0),
            bias=True,
    ...
    def feature_maps_to_wav(
        ...
        # Reshape.
        waveform = x.reshape(batch_size, self.target_sources_num * self.input_channels, audio_length)
        # (batch_size, target_sources_num * input_channels, segments_num)
